...
{\large
\begin{tabular}{@{}lcl@{}}
Ayush Yadav       & \quad--\quad & CS24BT001\\[2mm]
...
The submission has the following layout:
...
CS24BT001_CS24BT022_Assignment3_report.pdf    this report
\end{lstlisting}

The five workload description files provided with the assignment are not part of the
submission. The makefile expects them, named \texttt{process1.txt} to
\texttt{process5.txt}, in a \texttt{processes/} directory placed next to \texttt{src/}:
...
make run-round-robin-single       # Round Robin
...
make run-round-robin-dual         # Round Robin
...
make run-round-robin-dual FILE_NUMBER=5 TIME_QUANTUM=8
...
\paragraph{Step 1 --- compile.} From inside \texttt{src/}:
...
\paragraph{Dual processor.} The output has one schedule block per processor, each preceded
by a blank line, followed by the metrics, which include the run time of each processor:
...
6.7\,\% on workload 1, from 13.7\,\% to 10.9\,\% on workload 2, from 10.0\,\% to 2.1\,\%
on workload 4 and from 14.1\,\% to 5.8\,\% on workload 5. With more processing capacity, the order in which processes are
...
made workload 5 2.48\,\% \emph{worse}. The boost is a remedy for starvation in the low-priority
queues; with a second processor there is much less starvation to remedy, while the
...
\midrule
FIFO & Best average TAT on every workload (tied on W3); highest worst-case turnaround and lowest utilisation on W4 & Best average TAT on W1, W2, W5; worst on W4 & Smallest gains (6.9--47.6\,\%); only policy to reach $2\times$ speedup \\ \addlinespace
